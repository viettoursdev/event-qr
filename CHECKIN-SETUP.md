...
4. **Authentication** → *Get started* → bật **Email/Password**.
   ...
   👉 Cũng trong **Authentication → Sign-in method**, bật thêm **Anonymous (Ẩn danh)**.
   Cần cái này để khách bấm **"Xác nhận tham dự"** trên trang QR ghi được trạng thái
   (khách không đăng nhập, dùng phiên ẩn danh; Rules chỉ cho ghi đúng 3 trường xác nhận).
...
- **Xác nhận tham dự (RSVP):** khách bấm trên trang QR → ghi qua phiên *ẩn danh*, Rules chỉ cho sửa 3 trường `confirmed/confirmedAt/confirmedVia`, KHÔNG đọc được danh sách hay SĐT. Đây là sự kiện nội bộ nên chấp nhận; nếu cần chặt hơn (chống bấm hộ theo STT) thì phải dùng Cloud Functions.
